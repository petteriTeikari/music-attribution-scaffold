...
>
> **Last updated:** 2026-02-15 | **Figures:** 187 (+ 24 pending generation)
...
## Academic Supplementary (fig-supplementary-*)

### fig-supplementary-01: Hero Landing Page

**Alt text:** Landing page screenshot: editorial typography with coral accents on warm cream background

**Caption:** *The scaffold's landing page demonstrates the editorial design philosophy: Instrument Serif headings, warm cream surfaces, fixed sidebar navigation with rotated text, and coral red accent elements.*

**Embed:**
```markdown
![Landing page screenshot: editorial typography with coral accents on warm cream background](docs/figures/repo-figures/assets/fig-supplementary-01-hero-landing-page.jpg)
```
...
**Alt text:** Work catalog screenshot: 8 Imogen Heap works with confidence and assurance indicators

**Caption:** *The work catalog displays Imogen Heap mock data in horizontal rows with confidence scores (green >= 0.85, amber 0.50-0.84, red < 0.50) and assurance level badges (A0-A3), demonstrating the editorial row layout with dividers rather than card-based design.*

**Embed:**
```markdown
![Work catalog screenshot: 8 Imogen Heap works with confidence and assurance indicators](docs/figures/repo-figures/assets/fig-supplementary-02-work-catalog-view.jpg)
```
...
**Alt text:** Work detail screenshot: confidence gauge with source provenance and assurance breakdown

**Caption:** *The work detail view for "Hide and Seek" (0.87 confidence, A2 Corroborated) shows the ConfidenceGauge component with role="meter" accessibility, source-by-source provenance breakdown across MusicBrainz, Discogs, AcoustID, file metadata, and artist input, with per-source confidence scores.*

**Embed:**
```markdown
![Work detail screenshot: confidence gauge with source provenance and assurance breakdown](docs/figures/repo-figures/assets/fig-supplementary-03-work-detail-confidence.jpg)
```
...
**Alt text:** Review queue screenshot: AI attribution suggestions as diffs with batch approval controls

**Caption:** *The attribution review queue surfaces AI suggestions as before/after diffs grouped by work, with individual and batch approval controls and a progress counter showing review momentum -- designed as the key friction reducer per the UX-first philosophy.*

**Embed:**
```markdown
![Review queue screenshot: AI attribution suggestions as diffs with batch approval controls](docs/figures/repo-figures/assets/fig-supplementary-04-attribution-review-queue.jpg)
```
...
**Alt text:** Permissions dashboard screenshot: per-work AI use controls with allow/deny/conditional states

**Caption:** *The MCP permissions dashboard enables rights holders to set granular per-work, per-use-type consent (streaming, sync license, download, AI training, voice cloning) with three permission states -- ALLOW, DENY, and CONDITIONS -- served as machine-readable preferences via the MCP check_permissions tool.*

**Embed:**
```markdown
![Permissions dashboard screenshot: per-work AI use controls with allow/deny/conditional states](docs/figures/repo-figures/assets/fig-supplementary-05-mcp-permissions-dashboard.jpg)
```
...
**Alt text:** Agent chat sidebar screenshot: natural language attribution confidence explanation

**Caption:** *The CopilotKit agent chat sidebar enables natural language attribution queries. Asking "Why is this work rated 0.87?" triggers the explain_confidence tool, which returns a source-by-source breakdown with assurance level context -- powered by PydanticAI with claude-haiku-4-5 default.*

**Embed:**
```markdown
![Agent chat sidebar screenshot: natural language attribution confidence explanation](docs/figures/repo-figures/assets/fig-supplementary-06-agent-chat-sidebar.jpg)
```
...
**Alt text:** Academic print PRD: 78-node Bayesian decision network across 5 levels for print format

**Caption:** *The complete 78-node Bayesian decision network spanning 5 levels (L1 Business through L5 Operations) with approximately 131 conditional probability edges, adapted for academic print format. Core infrastructure (50 nodes) and ecosystem integration (28 nodes) are distinguished by fill style for B&W readability.*

**Embed:**
```markdown
![Academic print PRD: 78-node Bayesian decision network across 5 levels for print format](docs/figures/repo-figures/assets/fig-supplementary-07-full-prd-network-academic.jpg)
```
...
**Alt text:** Academic ecosystem map: four-tier company landscape with 28 PRD node annotations

**Caption:** *The music AI attribution ecosystem mapped across four tiers with 28 PRD ecosystem node annotations showing integration surfaces. Line styles differentiate three integration archetypes: Simple MCP (thin), Platform (medium), and CMO Federation (thick) -- adapted from fig-repo-16 and fig-ecosystem-01 for academic print.*

**Embed:**
```markdown
![Academic ecosystem map: four-tier company landscape with 28 PRD node annotations](docs/figures/repo-figures/assets/fig-supplementary-08-ecosystem-integration-academic.jpg)
```
